<template>
    <div class="task-scripting">
      <Toolbox :aiTools="aiTools" />
      <Canvas :tasks="tasks" @addTask="addTask" @removeTask="removeTask" />
  
      <div class="workflow-actions">
        <button @click="saveWorkflow">{{ $t('save_workflow') }}</button>
        <button v-if="selectedWorkflow !== null" @click="renameWorkflow">{{ $t('rename_workflow') }}</button>
        <select v-model="selectedWorkflow" @change="loadWorkflow">
          <option v-for="(workflow, index) in savedWorkflows" :key="index" :value="index">
            {{ workflow.name }}
          </option>
        </select>
        <button v-if="selectedWorkflow !== null" @click="deleteWorkflow">{{ $t('delete_workflow') }}</button>
