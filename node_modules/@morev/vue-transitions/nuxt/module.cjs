  ...
  async setup(options, nuxt) {
    ...
    COMPONENTS.forEach((componentName) => {
      ...
      node_fs.writeFileSync(
        ...
        templateContents.replaceAll("<%= options.propsDeclaration %>", propsDeclaration).replaceAll("<%= options.listenersDeclaration %>", kit.isNuxtMajorVersion(2, nuxt) ? ' v-on="$listeners"' : "").replaceAll("<%= options.componentName %>", componentName)
      );
    });
