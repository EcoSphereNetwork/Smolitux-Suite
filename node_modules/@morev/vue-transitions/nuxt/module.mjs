...
import { defineNuxtModule, createResolver, isNuxtMajorVersion, addComponentsDir } from '@nuxt/kit';
  ...
  async setup(options, nuxt) {
    ...
    COMPONENTS.forEach((componentName) => {
      ...
      writeFileSync(
        ...
        templateContents.replaceAll("<%= options.propsDeclaration %>", propsDeclaration).replaceAll("<%= options.listenersDeclaration %>", isNuxtMajorVersion(2, nuxt) ? ' v-on="$listeners"' : "").replaceAll("<%= options.componentName %>", componentName)
      );
    });
    addComponentsDir({
